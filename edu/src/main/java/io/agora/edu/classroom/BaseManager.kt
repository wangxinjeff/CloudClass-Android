...
open class BaseManager(
    ...
    protected fun userRoleConvert(role: EduUserRole): AgoraUIUserRole {
        return when (role) {
            EduUserRole.TEACHER -> AgoraUIUserRole.Teacher
            EduUserRole.STUDENT -> AgoraUIUserRole.Student
            EduUserRole.ASSISTANT -> AgoraUIUserRole.Assistant
            else -> AgoraUIUserRole.Teacher
        }
    }

    protected fun userInfoConvert(info: EduBaseUserInfo): AgoraUIUserInfo {
        return AgoraUIUserInfo(info.userUuid, info.userName, userRoleConvert(info.role))
    }

    protected fun muteLocalAudio(isMute: Boolean) {
        if (localStream != null) {
            switchLocalVideoAudio(localStream!!.hasVideo, !isMute)
        }
    }

    protected fun muteLocalVideo(isMute: Boolean) {
        if (localStream != null) {
    ...
    protected fun notifyUserDeviceState(info: AgoraUIUserDetailInfo, callback: EduCallback<Unit>) {
        if (info.user.userUuid == launchConfig.userUuid) {
            // Local
            // Temporarily default microphone available
            info.microState = AgoraUIDeviceState.Available
            // Determine if the camera is available
            if (info.enableVideo) {
                if (localCameraState == RteLocalVideoState.LOCAL_VIDEO_STREAM_STATE_FAILED.value) {
                    info.cameraState = AgoraUIDeviceState.UnAvailable
                } else {
                    info.cameraState = AgoraUIDeviceState.Available
                }
            } else {
                info.cameraState = AgoraUIDeviceState.Available
            }
