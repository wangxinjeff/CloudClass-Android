...
class AgoraUIHandsUp(
    ...
    private var curState: AgoraUIHandsUpState = AgoraUIHandsUpState.Init
    private var coHost = false
    var handsUpListener: IAgoraUIHandsUpListener? = null
    ...
    private var handsUpCountDownTimer: CountDownTimer = object : CountDownTimer(3200, 1000) {
        override fun onFinish() {
            ...
            handsUpListener?.onHandsUpState(AgoraUIHandsUpState.HandsUp)
        }
    ...
    private var cancelCountDownTimer: CountDownTimer = object : CountDownTimer(3200, 1000) {
        override fun onFinish() {
            countDownLayout.visibility = INVISIBLE
            countDownText.text = countDownTexts[0]
            handsUpListener?.onHandsUpState(AgoraUIHandsUpState.HandsDown)
        }
    ...
    override fun onTouch(v: View?, event: MotionEvent?): Boolean {
        when (event?.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (curState == AgoraUIHandsUpState.Init || curState == AgoraUIHandsUpState.HandsDown) {
                    handsUpCountDownTimer.start()
                } else if(curState == AgoraUIHandsUpState.HandsUp) {
