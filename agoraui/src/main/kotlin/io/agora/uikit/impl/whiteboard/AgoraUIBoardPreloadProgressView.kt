...
class AgoraUIBoardPreloadProgressView : LinearLayout {
    ...
    private var listener: WeakReference<BoardPreloadProgressJumpOverListener>? = null

    companion object {
        fun show(viewGroup: ViewGroup, url: String, listener: BoardPreloadProgressJumpOverListener?): AgoraUIBoardPreloadProgressView {
    ...
    fun timeout() {
        progressText?.let { it ->
            ...
            jumpOverText?.setOnClickListener {
                listener?.let {
                    listener!!.get()?.onJumpOver(this.url)
                    dismiss()
                }
            }
        }
...
interface BoardPreloadProgressJumpOverListener {
    fun onJumpOver(url: String?)
