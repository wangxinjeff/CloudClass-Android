...
import io.agora.uikit.interfaces.protocols.IAgoraUIVideo
...
import io.agora.uikit.impl.users.AgoraUIDeviceState
...
internal class AgoraUIVideo(
        ...
        left: Float,
        top: Float,
        shadowWidth: Float) : IAgoraUIVideo, View.OnClickListener {
    ...
    init {
        ...
        cardView.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                var audioSize = min((tmp * videoOptionIconSizePercent).toInt(), maxSize)
    ...
    private fun setCameraState(info: AgoraUIUserDetailInfo) {
        if (!info.onLine || info.cameraState == AgoraUIDeviceState.UnAvailable
                || info.cameraState == AgoraUIDeviceState.Closed) {
            videoIc.isEnabled = false
        } else {
    ...
    private fun setVideoPlaceHolder(info: AgoraUIUserDetailInfo) {
        ...
        if (!info.onLine || info.cameraState == AgoraUIDeviceState.Closed) {
            offLineLoadingLayout.visibility = VISIBLE
        } else if (!info.enableVideo) {
            videoOffLayout.visibility = VISIBLE
        } else if (info.enableVideo) {
            if (info.cameraState == AgoraUIDeviceState.UnAvailable) {
    ...
    private fun setMicroState(info: AgoraUIUserDetailInfo) {
        if (!info.onLine || info.microState == AgoraUIDeviceState.UnAvailable
                || info.microState == AgoraUIDeviceState.Closed) {
            audioIc.isEnabled = false
    ...
    override fun upsertUserDetailInfo(info: AgoraUIUserDetailInfo) {
        ...
        this.view.post {
            if (info.user.role == AgoraUIUserRole.Student) {
                ...
                boardGrantedIc.visibility = if (info.coHost && info.boardGranted) VISIBLE else GONE
                boardGrantedIc.visibility = if (info.coHost && info.boardGranted) VISIBLE else INVISIBLE
                setCameraState(info)
            ...
            val currentVideoOpen: Boolean = userDetailInfo?.let {
                it.onLine && it.enableVideo && it.cameraState == AgoraUIDeviceState.Available
            } ?: false

            val newVideoOpen = info.onLine && info.enableVideo && info.cameraState == AgoraUIDeviceState.Available
    ...
    override fun updateAudioVolumeIndication(volume: Int, streamUuid: String) {
        view?.post {
            volumeLayout.removeAllViews()
            var volumeLevel = 0
            if (volume > -1) {
                volumeLevel = (volume / 36.0f).toInt()
            }
            if (volumeLevel == 0) {
                volumeLevel = 1
