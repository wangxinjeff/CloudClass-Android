...
class AgoraUIToolDialog(context: Context,
    ...
    private fun initStyleDialog(anchor: View?) {
        ...
        anchor?.let { adjustPosition(it, width, height) }
    }
    ...
    private inner class ColorAdapter: RecyclerView.Adapter<ColorViewHolder>() {
        ...
        private val icons = List(iconColorStrings.size) { index ->
            makeStateList(
                    makeDrawable(Color.parseColor(iconColorStrings[index]), iconSize,
        ...
        private fun makeDrawable(color: Int, size: Int, selectColor: Int, selectWidth: Int,
...
interface AgoraUIToolDialogListener {
    fun onFontSizeSelected(index: Int)
