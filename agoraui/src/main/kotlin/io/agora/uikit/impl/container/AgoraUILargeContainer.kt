...
class AgoraUILargeClassContainer(
    ...
    private val largeContainerUserHandler = object : UserHandler() {
        override fun onUserListUpdated(list: MutableList<EduContextUserDetailInfo>) {
            ...
            // refresh possible assistantVideos
            val assistantVideos = list.filter { it.user.role == EduContextUserRole.Assistant }
            if (assistantVideos.isNotEmpty()) {
                assistantVideo?.upsertUserDetailInfo(assistantVideos[0])
            }
        }

        override fun onCoHostListUpdated(list: MutableList<EduContextUserDetailInfo>) {
            super.onCoHostListUpdated(list)
            // refresh studentVideos
    ...
    private var assistantVideo: AgoraUIVideo? = null
    ...
    override fun init(layout: ViewGroup, left: Int, top: Int, width: Int, height: Int) {
        ...
        // add assistantVideo demo
        assistantContainer = LinearLayout(layout.context)
        val params = RelativeLayout.LayoutParams(teacherVideoW / 2, teacherVideoH / 2).apply {
            leftMargin = teacherVideoLeft - this.width
            topMargin = teacherVideoTop
            assistantContainer?.layoutParams = this
        }
        layout.addView(assistantContainer)
        assistantContainer?.let {
            assistantVideo = AgoraUIVideo(layout.context, it, 0.0f, 0.0f, 0.0f)
            assistantVideo?.videoListener = object : IAgoraUIVideoListener {
                ...
                override fun onRendererContainer(viewGroup: ViewGroup?, streamUuid: String) {
                    getEduContext()?.videoContext()?.renderVideo(viewGroup, streamUuid)
                }
            }
        }
    ...
    override fun resize(layout: ViewGroup, left: Int, top: Int, width: Int, height: Int) {
        ...
        val params = assistantContainer?.layoutParams as RelativeLayout.LayoutParams
        params?.let {
            it.leftMargin = teacherVideoLeft
            it.topMargin = teacherVideoTop
            it.width = teacherVideoW / 2
            it.height = teacherVideoH / 2
            assistantContainer?.layoutParams = it
        }
